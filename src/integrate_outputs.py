...
def main(args):
    salmon_all_outputs = add_output_columns(args.salmon_all_sorted, args.event_outputs)
    salmon_all_outputs.to_csv('all_classifier_outputs.bed', sep='\t', index=False, header=True)

    integrate_multiple_calls('all_classifier_outputs.bed', args.smoothing_windows, "integrated_output.bed")

    integrated_df = pd.read_csv("integrated_output.bed", delimiter='\t', header=0)
    ...
    filtered_df.to_csv("merged_calls_ho.bed", sep='\t', index=False, header=False)
